...
Pw=[0.9 -0.3 0.7 -0.28 0.5 -0.25 0.36 -0.22 0.3 -0.18 0.28 -0.15 0.22 -0.1 0.2...
    -0.1 0.14 -0.08 0.1 -0.05 0.05 -0.03 0.02 -0.01 0.01 -0.01];
% Pw = exp(-(1:length(P))).*P;
% Sw = fir1(13,0.3);
Sw=Pw(13:end)*0.6;
...
figure
interval = [(1:0.125*T);(0.125*T:0.25*T-1) ;(0.25*T:0.375*T-1); (0.375*T:0.5*T-1);...
    (0.5*T:0.625*T-1);(0.625*T:0.75*T-1);(0.75*T:0.875*T-1);(0.875*T:T-1)];
for i = 1:8
