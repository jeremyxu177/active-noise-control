function result_time_domain=fxlms_time_domain(k)
...
% Pw=[0.9 0.9 -0.3 -0.3 0.7 0.7 -0.28 -0.28 0.5 0.5 -0.25 -0.25 0.36 0.36 -0.22 -0.22...
%     0.3 0.3 -0.18 -0.18 0.28 0.28 -0.15 -0.15 0.22  0.22 -0.1 -0.1 0.2 0.2 -0.1 -0.1...
%     0.14 0.14 -0.08 -0.08 0.1 0.1 -0.05 -0.05 0.05 0.05 -0.03 -0.03 0.02 0.02 -0.01 -0.01];
Sw=Pw(13:end)*0.6;
% Pw = fir1(24,0.4);
% Sw = fir1(12,0.2);
...
%     S_hat_w=S_hat_w + mu*e_iden(n)*S_hat_x + error(n);   % adjust the weight
    S_hat_w=S_hat_w + mu*e_iden(n)*S_hat_x;
end
...
X=cos(2*pi*(1/4)*[0:T-1])+cos(2*pi*(1/100)*[0:T-1]);
n  = 0.03*randn(1,T);  % Observation noise signal
d = filter(Pw,1,X)+n; % desired signal (signal at the error microphone)
...
ratio = zeros(1,T-100);
for i = 1:T-100
    ratio(i) = (sum(e(i:(i+99)).^2))/(sum(d(i:(i+99)).^2));
end

% ratio = zeros(1,T-10);
% for i = 1:T-10
%     ratio(i) = (sum(e(i:(i+9)).^2))/(sum(d(i:(i+9)).^2));
% end
